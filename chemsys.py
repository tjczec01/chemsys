...
class symbolgen:
       ...
       def dislat(lnames, latexs, indvar):
              ...
              for i in range(len(latexs)):
                     dd = '{d'+ 'C_{}'.format(sp.symbols(lnames[i]))+ '}'
       ...
       def fullgen(names, rxn, inits, eqs, intz, filepathf, kk, ea, r):
              ...
              EAK = KS.copy()
              EAK.extend(EAS)
              ff = symbolgen.fsave(filepathf, equats, dlat, Chem, ChemW, RHS, RHSf, Jac, JacN, Jacm, lm, latexmatrix, JacS, JacSN, JacSm, lmS, latexmatrixS, Cs, EAK)
