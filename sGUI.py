...
def close_window():
     ...
     indvdf.append(entry3)
     entry4 = str(filev.get())
...
class symbolgen:
       ...
       def __init__(self, nlist,  Initlist, EQlist, intvar):
       ...
       def gen(names, inits, eqs, intvarg):
              rxn = len(names)
              ...
              slat, dlat = symbolgen.dislat(names, latexss, intvarg)
              return Cs, equats, slat, dlat
       
       def psave(nameslist, LATEXD, filepathn):
              filename = filepathn
...
filepaths = ffpath[0]
C, equations, slat, dlat = symbolgen.gen(chemnamesl, Initreactions, Eqlist, indvdf[0])
print(C)
print(equations)
symbolgen.psave(chemnamesl, dlat, filepaths)
