...
class gui:
    ...
    def fullgui(self):
        chemnumslf, rxnsvl, indvdf, RR, ffpath = self.first()
        chems_value = chemnumslf[0]
        rxnnum = int(rxnsvl[0])
        rxnnumr = [int(i + 1) for i in range(rxnnum)]
        chemnamesl = self.second(chems_value)
        reactants_num, products_num, kk, eaf, reverse = self.third(rxnnumr, rxnnum)
        Initreactions, Eqlist = self.fourth(chemnamesl, rxnnum, reactants_num, products_num, reverse)
...
class symbolgen:
    ...
    def csave(LATEXC, fpath):
        ...
        for s, k in enumerate(LATEXC):
            ...
            ax = fig6.add_axes([0, 0, 0.001, 0.001])
            left, width = .25, .5
            bottom, height = .25, .5
            right = left + width
            top = bottom + height
        ...
        for s, k in enumerate(LATEXC):
            ...
            ax = fig7.add_axes([0, 0, 0.001, 0.001])
            left, width = .25, .5
            bottom, height = .25, .5
            right = left + width
            top = bottom + height
...
GG = gui()
chemical_names, number_of_reactions, Initial_reactions, Equation_list, indvdf, filepath, kvalues, ea_values, r_gas = GG.fullgui()  # Generates all necessary lists and values.
